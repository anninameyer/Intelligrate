...
def _run_once(cfg: dict, *, data_dir: Path, out_dir: Path) -> dict:
    ...
    model_bray_union_legacy = bray_spearman_union(Y, oof_tss, detect_threshold=union_detect, fillna_zero=False)
    model_proc_ait_raw = procrustes_union_aitchison(
        Y, oof_tss, pseudocount=union_pseudo, detect_threshold=0.0, fillna_zero=True
    )
    model_proc_ait = procrustes_union_aitchison(
        Y, oof_tss, pseudocount=union_pseudo, detect_threshold=union_detect, fillna_zero=True
    )
    model_proc_ait_legacy = procrustes_union_aitchison(
        Y, oof_tss, pseudocount=union_pseudo, detect_threshold=union_detect, fillna_zero=False
    )
    model_proc_bray_raw = procrustes_union_bray(Y, oof_tss, detect_threshold=0.0, fillna_zero=True)
    model_proc_bray = procrustes_union_bray(Y, oof_tss, detect_threshold=union_detect, fillna_zero=True)
    model_proc_bray_legacy = procrustes_union_bray(Y, oof_tss, detect_threshold=union_detect, fillna_zero=False)
    ...
    if picrust2 is not None:
        ...
        picrust2_bray_union = bray_spearman_union(Y, picrust2, detect_threshold=union_detect, fillna_zero=True)
        picrust2_bray_union_legacy = bray_spearman_union(Y, picrust2, detect_threshold=union_detect, fillna_zero=False)
        picrust2_proc_ait_raw = procrustes_union_aitchison(
            Y, picrust2, pseudocount=union_pseudo, detect_threshold=0.0, fillna_zero=True
        )
        picrust2_proc_ait = procrustes_union_aitchison(
            Y, picrust2, pseudocount=union_pseudo, detect_threshold=union_detect, fillna_zero=True
        )
        picrust2_proc_ait_legacy = procrustes_union_aitchison(
            Y, picrust2, pseudocount=union_pseudo, detect_threshold=union_detect, fillna_zero=False
        )
        picrust2_proc_bray_raw = procrustes_union_bray(Y, picrust2, detect_threshold=0.0, fillna_zero=True)
        picrust2_proc_bray = procrustes_union_bray(Y, picrust2, detect_threshold=union_detect, fillna_zero=True)
        picrust2_proc_bray_legacy = procrustes_union_bray(
    ...
    full_fit_bray_union = bray_spearman_union(Y, full_fit_tss, detect_threshold=union_detect, fillna_zero=True)
    full_fit_bray_union_legacy = bray_spearman_union(
        Y, full_fit_tss, detect_threshold=union_detect, fillna_zero=False
    )
    full_fit_proc_ait_raw = procrustes_union_aitchison(
        Y, full_fit_tss, pseudocount=union_pseudo, detect_threshold=0.0, fillna_zero=True
    )
    full_fit_proc_ait = procrustes_union_aitchison(
        Y, full_fit_tss, pseudocount=union_pseudo, detect_threshold=union_detect, fillna_zero=True
    )
    full_fit_proc_ait_legacy = procrustes_union_aitchison(
        Y, full_fit_tss, pseudocount=union_pseudo, detect_threshold=union_detect, fillna_zero=False
    )
    full_fit_proc_bray_raw = procrustes_union_bray(Y, full_fit_tss, detect_threshold=0.0, fillna_zero=True)
    full_fit_proc_bray = procrustes_union_bray(Y, full_fit_tss, detect_threshold=union_detect, fillna_zero=True)
    full_fit_proc_bray_legacy = procrustes_union_bray(
        Y, full_fit_tss, detect_threshold=union_detect, fillna_zero=False
    )
...
def main():
    ...
    print(f"MODEL_DM_UNION={float(run['model_dm_union']):.6f}")
    print(f"MODEL_DM_UNION_STRICT={float(run['model_dm_union_strict']):.6f}")
    print(f"FULL_FIT_DM_UNION={float(run['full_fit_dm_union']):.6f}")
    print(f"MODEL_BRAY_UNION_RAW={float(run['model_bray_union_raw']):.6f}")
    print(f"MODEL_BRAY_UNION={float(run['model_bray_union']):.6f}")
    print(f"MODEL_BRAY_UNION_LEGACY={float(run['model_bray_union_legacy']):.6f}")
    print(f"MODEL_PROC_AITCHISON_RAW={float(run['model_procrustes_aitchison_raw']):.6f}")
    print(f"MODEL_PROC_AITCHISON={float(run['model_procrustes_aitchison']):.6f}")
    print(f"MODEL_PROC_AITCHISON_LEGACY={float(run['model_procrustes_aitchison_legacy']):.6f}")
    print(f"MODEL_PROC_BRAY_RAW={float(run['model_procrustes_bray_raw']):.6f}")
    print(f"MODEL_PROC_BRAY={float(run['model_procrustes_bray']):.6f}")
    print(f"MODEL_PROC_BRAY_LEGACY={float(run['model_procrustes_bray_legacy']):.6f}")
    print(f"FULL_FIT_DM_UNION_RAW={float(run['full_fit_dm_union_raw']):.6f}")
    print(f"FULL_FIT_BRAY_UNION={float(run['full_fit_bray_union']):.6f}")
    print(f"FULL_FIT_BRAY_UNION_RAW={float(run['full_fit_bray_union_raw']):.6f}")
    print(f"FULL_FIT_PROC_AITCHISON={float(run['full_fit_procrustes_aitchison']):.6f}")
    print(f"FULL_FIT_PROC_AITCHISON_RAW={float(run['full_fit_procrustes_aitchison_raw']):.6f}")
    print(f"FULL_FIT_PROC_BRAY={float(run['full_fit_procrustes_bray']):.6f}")
    print(f"FULL_FIT_PROC_BRAY_RAW={float(run['full_fit_procrustes_bray_raw']):.6f}")
    print(f"FULL_FIT_DM_UNION_LEGACY={float(run['full_fit_dm_union_legacy']):.6f}")
    print(f"FULL_FIT_BRAY_UNION_LEGACY={float(run['full_fit_bray_union_legacy']):.6f}")
    print(f"FULL_FIT_PROC_AITCHISON_LEGACY={float(run['full_fit_procrustes_aitchison_legacy']):.6f}")
    print(f"FULL_FIT_PROC_BRAY_LEGACY={float(run['full_fit_procrustes_bray_legacy']):.6f}")
    if run.get("picrust2_dm_union") is not None:
        print(f"PICRUST2_DM_UNION_RAW={float(run['picrust2_dm_union_raw']):.6f}")
        print(f"PICRUST2_DM_UNION={float(run['picrust2_dm_union']):.6f}")
        print(f"PICRUST2_DM_UNION_LEGACY={float(run['picrust2_dm_union_legacy']):.6f}")
        print(f"PICRUST2_BRAY_UNION_RAW={float(run['picrust2_bray_union_raw']):.6f}")
        print(f"PICRUST2_BRAY_UNION={float(run['picrust2_bray_union']):.6f}")
        print(f"PICRUST2_BRAY_UNION_LEGACY={float(run['picrust2_bray_union_legacy']):.6f}")
        print(f"PICRUST2_PROC_AITCHISON_RAW={float(run['picrust2_procrustes_aitchison_raw']):.6f}")
        print(f"PICRUST2_PROC_AITCHISON={float(run['picrust2_procrustes_aitchison']):.6f}")
        print(f"PICRUST2_PROC_AITCHISON_LEGACY={float(run['picrust2_procrustes_aitchison_legacy']):.6f}")
        print(f"PICRUST2_PROC_BRAY_RAW={float(run['picrust2_procrustes_bray_raw']):.6f}")
        print(f"PICRUST2_PROC_BRAY={float(run['picrust2_procrustes_bray']):.6f}")
        print(f"PICRUST2_PROC_BRAY_LEGACY={float(run['picrust2_procrustes_bray_legacy']):.6f}")
